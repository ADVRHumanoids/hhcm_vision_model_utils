...
# -*- coding: utf-8 -*-
"""
Extract COCO Dataset Subset and Export to YOLO Format.

This script extracts a subset of the COCO dataset (train/val) for specified categories
and converts annotations to YOLO format (detection or segmentation). It handles:

- Category-based sampling with guaranteed minimum instances per class
- Automatic image downloading from COCO URLs or local copy
- YOLO format conversion (detection: bbox, segmentation: polygons)
- Dataset split management (train/val with cross-filling)
- YAML configuration generation for YOLO training

Features:
- Parallel image downloading with progress bars
- Normalized coordinates (0-1) for YOLO compatibility
- Automatic handling of insufficient validation data
- Duplicate removal and data integrity checks

Author: tori (originally from Colab -
      https://colab.research.google.com/drive/1qr5UsOKCY0-_70VTER7Z3KJu6zGUqET1
    )

Configuration:
    Edit the following variables in the script:
    - my_categories: Dict mapping category names to new class IDs
    - N_sample_train/N_sample_valid: Minimum instances per category
    - model_type: "det" or "seg" for detection or segmentation
    - cocoTrain/cocoVal: Paths to COCO annotation JSON files
    - img_folder: Path to COCO images directory
    - output_name: Base path for output dataset
